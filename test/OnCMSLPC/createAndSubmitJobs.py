...
Output = cmsRun_$(Cluster)_$(Process).stdout
Error = cmsRun_$(Cluster)_$(Process).stderr
Log = cmsRun_$(Cluster)_$(Process).log
notify_user = devdatta@FNAL.GOV
Queue 1
"""
...
cmsRun CMSSW_cfg.py CFG_PARAMETERS outFilename=bprimeTobH_DATASETNAME_JOB_NUMBER.root 
exitcode=$?

#cp -v OUTPUT_FILENAME.root DATASET_WORKDIR/output/OUTPUT_FILENAME_JOB_NUMBER.root
cp -v bprimeTobH_DATASETNAME_JOB_NUMBER.root DATASET_WORKDIR/output/OUTPUT_FILENAME_JOB_NUMBER.root
...
def main():
  ...
  for line in dataset_list_lines:
    ...
    for ijob in range(ijobmax):
       ...
       bash_script_content = re.sub('DATASETNAME',dataset,bash_script_content)
       ...
       condor_script = open(os.path.join(dataset_workdir,'input','job_' + str(ijob) + '.jdl'),'w')
       condor_script_content = re.sub('MAIN_WORKDIR',main_workdir,condor_template)
       condor_script_content = re.sub('BASHSCRIPT',os.path.join(dataset_workdir,'input','job_' + str(ijob)),condor_script_content) 
       ...
       if(not options.no_submission):
         ...
         cmd = 'condor_submit ' + os.path.join(dataset_workdir,'input','job_' + str(ijob) + '.jdl')
         os.system(cmd)
         #cmd1 = 'cd ' + os.path.join(dataset_workdir,'input')
         #cmd2 = 'condor_submit job_' + str(ijob) + '.jdl'
         #cmd3 = 'cd ' + cfg_dirname 
         #os.system(cmd1)
         #os.system(cmd2) 
         #os.system(cmd3) 
